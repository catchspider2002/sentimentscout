...
**Stack:** Cloudflare Workers + Cron + D1 + DeepInfra LLM. No Container. News via Google News RSS and Reddit JSON (no keys).
...
- **Analyse** (`src/analyser.ts`): DeepInfra LLM compares sentiment to the opening odds and returns a strict JSON signal card (signals, mismatch, key factors, confidence). Deterministic fallback if no key; confidence forced low when < 5 sources.
...
wrangler secret put DEEPINFRA_API_KEY    # optional (LLM analysis; fallback without it)
npm run deploy
```
